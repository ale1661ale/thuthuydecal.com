        ...
        <table id="products_table" class="table display responsive nowrap">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <a href="{{ route('products.list', $value->id) }}" data-id="{{ $value->id }}" class="btn btn-success">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form action="{{ route('products.destroy', $product[0]->id) }}" method="post">
                ...
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Bạn có muốn xóa <span class="categoryName">{{ $product[0]->name }}</span></h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
